...
public class ModDetailSettings extends GuiScreen {
	...
	public void initGui() {
		...
		if(name.equals("NickHider")) {
					...
					if(select) {
						...
						if(Client.getInstance().modmanager.nickhider.setProfile(username)) {
							...
							if(Client.getInstance().modmanager.nickhider.skin) {
								((IMixinManager)Minecraft.getMinecraft().thePlayer).loadCustomSkin(Client.getInstance().modmanager.nickhider.profile);
							}
						}
					}
				}
			...
			base.add(new ToggleButton("Enable Skin Changes", 1120, 420, 100, 60, Client.getInstance().modmanager.nickhider.skin) {
				...
				public void doThings() {
					...
					if(select) {
						...
						((IMixinManager)Minecraft.getMinecraft().thePlayer).loadCustomSkin(((IMixinSession)Minecraft.getMinecraft()).getSession().getProfile());
					}
				}
			});
			return;
			...
			base.add(new ToggleButton("Animation of Floating Corpse", 1120, 440, 100, 60, Client.getInstance().modmanager.killeffect.floatc) {
				@Override
				public void doThings() {
					Client.getInstance().setting.writer("KillEffect", "float_cadaver", String.valueOf(select));
					Client.getInstance().modmanager.killeffect.floatc = select;}
			});
			base.add(new ToggleButton("Animation of Hiding Corpse", 1120, 540, 100, 60, Client.getInstance().modmanager.killeffect.hide) {
				@Override
				public void doThings() {
			...
			base.add(new ToggleButton("Enable Bypass Mode", 1120, 640, 100, 60, Client.getInstance().modmanager.killeffect.bypass) {
				@Override
				public void doThings() {
			...
			base.add(new ToggleButton("Add Recent Colors to Firework", 1120, 740, 100, 60, Client.getInstance().modmanager.killeffect.recent) {
				@Override
				public void doThings() {
			...
			charts.add(new ColorChart("Color of HUD", 960, 310, 255, 40, mod.getImageX(), mod.getImageY(), mod.getColorValue()) {
				@Override
				public void doThings() {

					Client.getInstance().setting.writer(mod.getName(), "red", String.valueOf(r));
					Client.getInstance().setting.writer(mod.getName(), "green", String.valueOf(g));
					Client.getInstance().setting.writer(mod.getName(), "blue", String.valueOf(b));
					
					Client.getInstance().setting.writer(mod.getName(), "imagex", String.valueOf(image_x));
					Client.getInstance().setting.writer(mod.getName(), "imagey", String.valueOf(image_y));
					
					mod.setColor(r, g, b); mod.setImageX(image_x); mod.setImageY(image_y);
			...
			base.add(new Bar("Size of HUD", 960, 310, 255, 90, mod.getSizeValue(), 0, 255) {
				@Override
				public void doThings() {
					int custom = ScaleFixer.disWB(x_point);
					Client.getInstance().setting.writer(mod.getName(), "size", String.valueOf(custom));
					mod.setSize(custom);}
			});
		...
		base.add(new Bar("Size of HUD", 960, 400, 255, 90, mod.getSizeValue(), 0, 255) {
			@Override
			public void doThings() {
				int custom = ScaleFixer.disWB(x_point);
				Client.getInstance().setting.writer(mod.getName(), "size", String.valueOf(custom));
