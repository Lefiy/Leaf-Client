...
public class ClientSettings extends GuiScreen {
	...
	public void initGui() {
		...
		base.add(new ToggleButton("Hide NameTag Icon", 1090, 710, 100, 60, Client.getInstance().modmanager.hideIcon) {
			@Override
			public void doThings() {
				Client.getInstance().setting.writer("Setting", "hide_icon", String.valueOf(select));
				Client.getInstance().modmanager.hideIcon = select;}
		});
		base.add(new ToggleButton("Hide Cape Cosmetics", 1090, 310, 100, 60, Client.getInstance().modmanager.hideCape) {
			@Override
			public void doThings() {
				Client.getInstance().setting.writer("Setting", "hide_cape", String.valueOf(select));
				Client.getInstance().modmanager.hideCape = select;}
		});
		base.add(new ToggleButton("Hide Wing Cosmetics", 1090, 410, 100, 60, Client.getInstance().modmanager.hideWing) {
			@Override
			public void doThings() {
				Client.getInstance().setting.writer("Setting", "hide_wing", String.valueOf(select));
				Client.getInstance().modmanager.hideWing = select;}
		});
		base.add(new ToggleButton("Hide Hat Cosmetics", 1090, 510, 100, 60, Client.getInstance().modmanager.hideHat) {
			@Override
			public void doThings() {
				Client.getInstance().setting.writer("Setting", "hide_hat", String.valueOf(select));
				Client.getInstance().modmanager.hideHat = select;}
		});
		base.add(new ToggleButton("Borderless Screen", 1090, 610, 100, 60, Client.getInstance().modmanager.borderless) {
			@Override
			public void doThings() {
				...
				if(select) {
					...
					try {
					    ...
					    Minecraft.getMinecraft().resize(w_screen, h_screen);
					...
					try {
					    ...
					    Minecraft.getMinecraft().resize(w_save, h_save);
					} catch (LWJGLException e) {}
				}
			}
		});
