...
public class ModButton extends UIBase {
	...
	@Override
	public void render() {
		GlStateManager.color(isCover ? 0 : mod.isEnable() ? 1 : 100, isCover ? 100 : mod.isEnable() ? 1 : 0, isCover ? 0 : mod.isEnable() ? 1 : 0, 1);
		...
		if(!mod.getName().equals("FreeLook") && !mod.getName().equals("Old Animation")) {
	...
	public void onMouseClick(int mouseX, int mouseY, int button) {
		if(mouseX >= x && mouseX <= x + w && mouseY >= y && mouseY <= y + (h / 2)) {
			...
			Client.getInstance().setting.writer(mod.getName(), "enable", String.valueOf(mod.isEnable()));
				...
				&& !(mod.getName().equals("FreeLook")) && !(mod.getName().equals("Old Animation"))) {
